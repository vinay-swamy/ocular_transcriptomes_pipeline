...
tibble(sample=names(k), percent_missing=k)
...
total_tss_quant <-  gtf_tss[,-(1:8)] %>% group_by(id) %>% summarise_all(sum) %>% .[,sample_table$sample]
...
```{r}
rpe_spec_tss <- res %>% inner_join(gtf_tss %>% select(transcript_id, id),. )
rpe_spec_tss_counts <- filter(quant_smoothed,transcript_id %in% rpe_spec_tss$id)
rpe_spec_gtf <- rtracklayer::readGFF('data/gtfs/filtered_tissue/RPE_Fetal.Tissue.gtf')

rpe_spec_tss_tx <- rpe_spec_tss %>% pull(id) %>% {filter(gtf_tss, id %in% .)} %>% pull(transcript_id) %>% {filter(tcons2mstrg, transcript_id %in% .)}
