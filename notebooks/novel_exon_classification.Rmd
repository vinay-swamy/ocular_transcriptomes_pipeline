...
```{r}
a3ss_files <- list.files('/Volumes/data/eyeintegration_splicing/rmats_out/', pattern = 'fromGTF.A3SS.txt', recursive = T, full.names = T )
a5ss_files <- list.files('/Volumes/data/eyeintegration_splicing/rmats_out/', pattern = 'fromGTF.A5SS.txt', recursive = T, full.names = T )
...
```{r}
se_files <- list.files('/Volumes/data/eyeintegration_splicing/rmats_out/', 'fromGTF.SE.txt', recursive = T, full.names = T)
k <- read_tsv(se_files[1])
...
```{r}
ri_files <- list.files('/Volumes/data/eyeintegration_splicing/rmats_out/', 'fromGTF.RI.txt', recursive = T, full.names = T)
...
most exons not detected in rMATs are first/last exons, this is what we expect, looks like we are getting a lot more SHTUFF
