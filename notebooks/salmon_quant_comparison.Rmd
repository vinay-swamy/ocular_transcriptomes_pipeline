...
subtissue <- sample_table$subtissue %>% unique()
proc_gtf <- function(tissue, build){
    gtf <- rtracklayer::readGFF(glue('data/gtfs/{build}/{tissue}_st.gtf'))
    num_ref <- gtf %>% filter(type =='transcript', grepl('ENST', transcript_id)) %>% nrow
    num_novel <- gtf %>% filter(type =='transcript', !grepl('ENST', transcript_id)) %>% nrow 
    if build == '
    return(tibble(tissue=tissue,))
}
