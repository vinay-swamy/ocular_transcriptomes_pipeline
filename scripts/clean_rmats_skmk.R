...
combine_PE_SE <- function(combination,event,files,event_header){
    ...
    if(length(target_files)==1){
        ...
        fold <- gsub('_PE|_SE','',target_files)
        path <- paste0('rmats_clean/', fold)
    ...
    path <- paste0('rmats_clean/',combination[1],'_VS_',combination[2])
    dir.create(path = path)
    write.table(final,paste(path,event,sep='/'),row.names = F,col.names = T, quote = F,sep = '\t')
...
for(j in 1:length(events)){
    i_event <- events[j]
    try(combine_PE_SE(combination = c(args[1],args[2]),event = i_event,files = i_files, event_header  = i_event_header),outFile = stdout())
    }
