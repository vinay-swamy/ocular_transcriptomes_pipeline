...
clinvar_sum <- read.table('ref/variant_summary_11.05.18.txt', stringsAsFactors = F, sep = '\t', header = T, comment.char = '', fill = T)%>%
     #dplyr::select(AlleleID,VariationID,Type,GeneSymbol,`RS...dbSNP.`,Origin,Assembly,Chromosome,Start,Stop)%>%
...
clinvar_unSig <- read_csv('ref/clinvar_uncertain_vars.csv') 
...
#######
clinvar_path <- read_csv('ref/clinvar_known_path.csv')
...
k[[4]][[1]] <- 23
k[[11]][[1]] <- 23
uk_rang <- GRanges(seqnames = lapply(k,function(x)x[1])%>%unlist, ranges = IRanges(start = lapply(k,function(x)x[2])%>%unlist,
                                                                                   end = lapply(k,function(x)x[3])%>%unlist))
findOverlaps(novel_exon_range,uk_rang)# nada pt 2

i <- c(rownames(rax_cts),rownames(f3_cts))
