...
process_events_for_tissue <- function(loc){
    ...
    proc_SE_RI_files <- function(file, lcols){
        tab <- read_tsv(file, col_types= "cccccdddddddccccddddccd")
        if(nrow(tab) == 0){
            cols_inc=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_inc') )
            cols_psi=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_psi') )
            dummy_inc <- matrix(data=0, nrow=0, ncol= length(cols_inc))
            colnames(dummy_inc) <- cols_inc
            dummy_psi <- matrix(data=0, nrow=0, ncol= length(cols_psi))
            colnames(dummy_psi) <- cols_psi
            return(list(incCounts=dummy_inc %>% as_tibble,
                        psi=dummy_psi %>%  as_tibble))
    ...
    proc_ASS_files <- function(file, long_name, short_name ){
        z <-"
        ...
        "
        tab <- read_tsv(file, col_types = 'dccccdddddddccccddddccd')
        if(nrow(tab) == 0){
            cols_inc=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_inc') )
            cols_psi=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_psi') )
            dummy_inc <- matrix(data=0, nrow=0, ncol= length(cols_inc))
    ...
    proc_MXE_files <- function(file, long_name, short_name ){
        z <-"
        It's a little confusing, but for the ASS files, the long isoform is the 'inlcluded' isoform, and the short is the 'excluded'.
        Therefore psi for the long form is inc/(inc+ex), and the psi for the short is ex/(inc+ex)
        "
        tab <- read_tsv(file, col_types = "cccccdddddddddccccddddccd")
        if(nrow(tab) == 0){
            cols_inc=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_inc') )
            cols_psi=c(c('seqid', 'strand', 'start', 'end'), paste0(samples, '_psi') )
            dummy_inc <- matrix(data=0, nrow=0, ncol= length(cols_inc))
    ...
    A3_all <- proc_ASS_files(event_files[[2]], 'long', 'short')
    A5_all <- proc_ASS_files(event_files[[3]],'long', 'short')
    ...
    MXE_all <- proc_MXE_files(event_files[[4]], '1st', '2nd')
    ...
    # this is really ugly and i want to change it 
    # but I feel like i put it there for a reason    
